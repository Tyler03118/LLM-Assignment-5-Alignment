...
def train(args):
    run_name = f"sft-size-{args.limit if args.limit else 'full'}-hf-eval"
    ...
    for epoch in range(args.epochs):
        ...
        for i in tqdm(range(0, len(prompts), args.batch_size), desc=f"Epoch {epoch+1}/{args.epochs}"):
            ...
            if global_train_step % args.eval_every == 0:
                print(f"\n[{global_train_step} 步] 进行 HF 原生评估...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CS336 Assignment 5 SFT Training (No vLLM)")
    ...
    parser.add_argument("--limit", type=int, default=None, help="限制训练数据量")
    parser.add_argument("--val_limit", type=int, default=100, help="限制验证集评估数量以防过慢")
